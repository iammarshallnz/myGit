...
class GitObject (object):
    ...
    def __init__(self, data=None):
        ...
        if data != None:
            ...
            # If no data is provided, we just initialize the object.
            self.init()
...
class GitBlob(GitObject):
    """serialize and deserialize functions just have to store and return their input unmodified."""
    fmt=b'blob'
...
def index_read(repo):
    ...
    if index_file is None:
        raise Exception(f"Could not determine path for index file in repository {repo.gitdir}")
...
def repo_create(path):
    ...
    # empty dir.
    if (repo.worktree == None ):
        raise Exception("Path is None!")
    if (repo.gitdir == None ):
        raise Exception("Gitdir is None!")
    ...
    # .git/description
    desc_path = repo_file(repo, "description") # repository file creation
    if desc_path is None:
        raise Exception("Could not determine path for .git/description")
    with open(desc_path, "w") as f:
        f.write("Unnamed repository; edit this file 'description' to name the repository.\n")

    # .git/HEAD
    repo_path = repo_file(repo, "HEAD") # repository file creation
    if repo_path is None:
        raise Exception("Could not determine path for .git/HEAD")
    with open(repo_path, "w") as f:
        f.write("ref: refs/heads/master\n")

    conf_path = repo_file(repo, "config") # repository file creation
    if conf_path is None:
        raise Exception("Could not determine path for .git/config")
...
def repo_find(path=".", required=True):
    """Find a git repository starting from path via working backwards ../ ."""
...
def object_read(repo, sha):
    ...
    path = repo_file(repo, "objects", sha[0:2], sha[2:]) # e673d1b7eaa0aa01b5bc2442d570a765bdaae751 is .git/objects/e6/73d1b7eaa0aa01b5bc2442d570a765bdaae751
    # If the object is not found, return None.
    if path is None:
        raise Exception(f"Object {sha} not found in repository {repo.gitdir}")
    
    if not os.path.isfile(path):
        return None # If the object is not a file, return None.
...
def object_write(obj, repo=None):
    ...
    if repo:
        ...
        if path is None:
            raise Exception(f"Could not determine path for object {sha} in repository {repo.gitdir}")
...
def cmd_init(args):
    ...
    print(f"Initializing new repository in {args.path}")
...
def log_graphviz(repo, sha, seen):
    ...
    if commit is None:  
        raise Exception(f"Commit {sha} not found in repository {repo.gitdir}")
    if not isinstance(commit, GitCommit): # if blob not kvlm
        raise Exception(f"Object {sha} is not a commit, but a blob")    
...
def cmd_checkout(args):
    ...
    if obj == None:
        raise Exception(f"Object {args.commit} not found in repository {repo.gitdir}")
...
def ref_resolve(repo, ref):
    ...
    if path is None:
        raise Exception(f"Reference {ref} not found in repository {repo.gitdir}")
...
def ref_list(repo, path=None):
    ...
    # listdir
    if path is None:
        raise Exception(f"Path {path} is not a directory in repository {repo.gitdir}")
...
def ref_create(repo, ref_name, sha):
    repo_obj = repo_file(repo, "refs/" + ref_name)
    if repo_obj is None:
        raise Exception(f"Could not determine path for reference {ref_name} in repository {repo.gitdir}")
    with open(repo_obj, 'w') as fp:
        fp.write(sha + "\n")
...
def cmd_ls_files(args):
    ...
    for e in index.entries:
        ...
        if args.verbose:
            ...
            print(f"  user: {os.getlogin()} ({e.uid})  group: {'''grp.getgrgid(e.gid).gr_name'''} ({e.gid})")
...
def gitignore_read(repo):
    ...
    if ret is None:
        raise Exception("Could not read gitignore rules.")
...
def check_ignore(rules, path):
    # Check both absolute and scoped rules
    return check_ignore1(rules.absolute, path) or check_ignore1(rules.scoped.get(os.path.dirname(path), []), path)
